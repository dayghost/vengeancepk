...
public class CombatAssistant{
	...
	public int[][] slayerReqs = {{1648,5},{1612,15},{1643,45},{1618,50},{1624,65},{1610,75},{1613,80},{1615,85},{2783,90}, {3590, 92}, {3591, 82}, {3592, 72}};
...
c.degradeSHelm();
	...
	**/
		...
		public void attackPlayer(int i) {
          ...
          c.gfx0(2138);
            ...
            for (int u : c.Bolts)  {
                for (int y : c.BOWS)  {
                    ...
                    /* DEGRADING */
          if (c.playerEquipment[c.playerWeapon] >= 13858) {
          c.degradeTime++;
          }
          if(c.playerEquipment[c.playerWeapon] == 13899 && c.degradeTime == 450) {// VLS Degrade.
          ...
          if(c.playerEquipment[c.playerWeapon] == 13902 && c.degradeTime == 500) {// STAT Degrade.
          c.getItems().wearItem(-1, 1, 3); // remove the equiped item
          c.sendMessage("Your Statius warhammer has degraded into nothing."); // send a message
          c.degradeTime = 0; // put the degrade number back to 0 so it can degrade again @ 500
          } 
          if(c.playerEquipment[c.playerWeapon] == 13905 && c.degradeTime == 450) {// Spear Degrade.
          c.getItems().wearItem(-1, 1, 3); // remove the equiped item
...
if((c.playerEquipment[c.playerHat] == -1) && (c.playerEquipment[c.playerCape] == -1) && (c.playerEquipment[c.playerAmulet] == -1) && (c.playerEquipment[c.playerChest] == -1) && (c.playerEquipment[c.playerShield] == -1) && (c.playerEquipment[c.playerLegs] == -1) && (c.playerEquipment[c.playerHands] == -1) && (c.playerEquipment[c.playerFeet] == -1) && (c.playerEquipment[c.playerWeapon] == -1)) {
		c.sendMessage("You can't attack with nothing equipped!");
					c.stopMovement();
					c.getCombat().resetPlayerAttack();
					return false;
		}
		if((Server.playerHandler.players[c.playerIndex].playerEquipment[Server.playerHandler.players[c.playerIndex].playerHat] == -1) && (Server.playerHandler.players[c.playerIndex].playerEquipment[Server.playerHandler.players[c.playerIndex].playerCape] == -1) && (Server.playerHandler.players[c.playerIndex].playerEquipment[Server.playerHandler.players[c.playerIndex].playerAmulet] == -1) && (Server.playerHandler.players[c.playerIndex].playerEquipment[Server.playerHandler.players[c.playerIndex].playerChest] == -1) && (Server.playerHandler.players[c.playerIndex].playerEquipment[Server.playerHandler.players[c.playerIndex].playerShield] == -1) && (Server.playerHandler.players[c.playerIndex].playerEquipment[Server.playerHandler.players[c.playerIndex].playerLegs] == -1) && (Server.playerHandler.players[c.playerIndex].playerEquipment[Server.playerHandler.players[c.playerIndex].playerHands] == -1) && (Server.playerHandler.players[c.playerIndex].playerEquipment[Server.playerHandler.players[c.playerIndex].playerFeet] == -1) && (Server.playerHandler.players[c.playerIndex].playerEquipment[Server.playerHandler.players[c.playerIndex].playerWeapon] == -1)) {
		c.sendMessage("You can't attack someone who has nothing equipped!");
		c.stopMovement();
		c.getCombat().resetPlayerAttack();
					return false;
		}
			...
			if (c.getPA().getWearingAmount() < 3 && c.duelStatus < 1) {
 ...
 if(weaponName.contains("2h sword") || weaponName.contains("godsword") || weaponName.contains("saradomin sw")) {
	...
	**/
	...
	public int calculateRangeAttack() {
		...
		attackLevel *= c.specAccuracy;
        if (c.fullVoidRange() || c.fullVoidEliteRange())
		...
		else if (c.fullVoidEliteRange() && c.specAccuracy > 1.15) {
			attackLevel *= 1.82;
		}
...
public void drawback() {
	...
	/*else if (c.playerEquipment[c.playerWeapon] == 13649){
		javerlin(105, 1190);
	}
	else if (c.playerEquipment[c.playerWeapon] == 13650){
		thaxe(105, 1195);
	}*/
...
if(c.playerEquipment[c.playerWeapon] == 15241)
	...
	public void handleDfs() {
		final Player opp = Server.playerHandler.players[c.playerIndex];
		if (opp == null) 
			return;
		if (System.currentTimeMillis() - c.dfsDelay > 30000) {
					if(c.dfsCount == 0) {
		...
		final int pX = c.getX();
		final int pY = c.getY();
		final int oX = opp.getX();
		final int oY = opp.getY();
		...
		EventManager.getSingleton().addEvent(new Event() {
			...
			public void execute(EventContainer b) {
				if (opp != null) {
					opp.gfx100(1167);
					if (opp.playerLevel[3] - damage < 0) {
						damage = opp.playerLevel[3];					
					}
					opp.playerLevel[3] -= damage;
					opp.hitDiff2 = damage;
					opp.hitUpdateRequired2 = true;
					opp.updateRequired = true;
				}
				b.stop();
			}
		}, 1700);
		EventManager.getSingleton().addEvent(new Event() {
			public void execute(EventContainer b) {
				if (c.playerIndex != 0)
					c.getPA().createPlayersProjectile2(pX, pY, offX, offY, 50, 50, 1166, 30, 30, - c.playerIndex - 1, 30, 5);
				b.stop();
			}
		}, 1000);
		c.dfsDelay = System.currentTimeMillis();
